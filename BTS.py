...
class BTS:
    def __init__(self, k_: int, s_: int, epsilon_: float, clock_: int, simulation_time_: int, _log: logging):
        ...
        self.t: float = 1  # random.exponential(scale=1 / 10)  # czas co ile pojawiają się nowi userzy
        ...
        self.user_list: List[User] = [User(_log=self.log, _rb=list()) for _ in range(15)]
    ...
    def redistribute_resource_blocks(self):  # Algorithm defined by A in exercise, work in progress, tmp solution
        # priorytet dla usera o throughput = 0 (wynika z matematyki)
        # przydzielamy bloki sekwencyjnie
        for user in self.user_list:
            user.clear_resource_blocks()

        for i in range(self.k):
            value, user_index = max(((divide(user.get_current_throughput(), user.get_avg_throughput()), user_index) for user_index, user in enumerate(self.user_list)), key=lambda x: x[0])
            picked = self.user_list[user_index]

            while picked.has_all_resource_blocks():
                value, user_index = max(((divide(user.get_current_throughput(), user.get_avg_throughput()), user_index) for user_index, user in enumerate(self.user_list)), key=lambda x: x[0])
                picked = self.user_list[user_index]
