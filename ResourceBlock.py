...
class ResourceBlock:
	def __init__(self, _log: logging, _epsilon: float) -> None:
		self.throughput: int = 400 # randint(low=20, high=800)  # [bit/ms] przepływność użytkownika
		self.epsilon: float = _epsilon
		self.log: logging.Logger = _log.getChild(__name__)
		self.is_sent: bool = random.random() >= self.epsilon # random_sample() >= self.epsilon
	...
	def update_is_sent(self) -> None:
		self.is_sent = random.random() >= self.epsilon # random_sample() >= self.epsilon

	def update_throughput(self):
		self.log.log(msg=f"Updating ResourceBlock's throughput to {self.throughput}", level=3)
		self.throughput = 400 # randint(low=20, high=800)  # przepływność użytkownika
